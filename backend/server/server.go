...
func New(cfg Config) *gin.Engine {
	...
	g := r.Group("/api", a.Middleware())
	{
		...
		g.GET("/mobile/devices", h.MobileDevices)  // 移动端设备 token 注册表（供推送服务使用）
		g.POST("/mobile/devices", h.RegisterMobileDevice)
		g.DELETE("/mobile/devices/:id", h.DeleteMobileDevice)
