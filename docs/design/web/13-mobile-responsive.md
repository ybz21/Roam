...
> **修订（2026-08）：浏览器 / 手机镜像放回底栏，底栏变 6 格。**
> 上面「低频」的判断错了：这两页是本机最常用的两个工具，藏在二级 sheet 里每次要点两下。
> 当初另一条理由「窄屏下几乎不可用」（地址栏固定 150、设备选择器固定 240）也已经不成立
> ——那两处早改成自适应了。现在底栏是
> `概览 / 项目 / 文件 / 浏览器 / 手机 / 更多`，360px 下每格 60px，图标 + 一行 11px 标签
> 单行截断（`.tt-bottomnav-btn`）。**格数到此为止**：再加一格标签就只能全省略，
> 那时底栏等于一排猜谜图标。「更多」sheet 里剩会话 / 插件 / 设置 + 账户段。
