...
| 21 | [21-session-pair/](./21-session-pair/index.html) | **拖一个会话到另一个会话里** — 把项目页的任务卡拖到终端坞的会话标签上，落下之后**往当前会话里注一段话，告诉它旁边那个是谁、怎么跟它说话**。就这些。通信方法是现成的 `ttmux send <会话> <消息>`（`Resolve` 认会话名/tmux id/展示名，投递走 `SendPromptSubmit`）——缺的只是 Agent 不知道有这条命令、也不知道对面是谁。注入的那段话是本稿唯一的设计物：对面身份 + 工作目录 + **会话 id**（展示名可重复）+ **自己的会话名当回信地址** + 「和人敲进去没区别」。**不建链路对象、不做配额/速率闸、不做往来记录、不加新命令、不弹确认框** —— 该说几句、什么时候停，大模型判断得比计数器好；往来就在两个 pane 里明摆着。唯一要拦的是跨机（`ttmux send` 走本机 tmux，找不到对方）|
