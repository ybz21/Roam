...
- **身份 = 不可变 id（v0.3 修订）**：项目 key 原是「目录名 slug + 路径 hash」，身份绑在
  路径上——`mv` 一下目录 key 就变，置顶/显示名/默认 agent 得靠 Rekey 搬家合并，URL 也换。
  现在入册时发一个 `YYYY-MMDD-HHMM-rand4` 的不可变 id（与蜂群 id 同款格式），**目录降级
  为可变属性**：子目录归位仓库根、用户挪目录都只改 `dir`（`Store.SetDir`），偏好与链接一律
  不动。老文件加载时自动迁移，老 key 记进 `aliases`，`#/projects/<老key>` 与 `GET/PATCH
  /projects/:key` 继续解析。目录查重靠 `byDir` 索引，不再靠路径 hash。
