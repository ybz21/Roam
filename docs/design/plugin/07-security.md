...
>
> Roami 的权限接近 SSH:插件跑在开发机上,以当前用户身份运行。安全设计必须默认保守,**并且诚实**。

## 1. v1 信任模型:安装即信任(与 VS Code 相同)

必须先说清一个技术事实:v1 的插件是**普通子进程**。进程隔离保护的是宿主的稳定性(崩溃不连带),**不构成沙箱**——插件代码自身可以执行任意命令、访问任意网络,宿主无法在进程模型下阻断。Figma 能真正阻断网络,是因为插件跑在浏览器沙箱里;Roami v1 不具备这个前提。
